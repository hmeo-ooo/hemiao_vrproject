...
[CustomPropertyDrawer(typeof(LevelAislePlacement))]
public class LevelAislePlacementDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.isExpanded)
        {
            ...
            float captureHeight = LevelPlacementCaptureUtility.DrawSceneCaptureField(
                ...
                property,
                LevelPlacementCaptureUtility.PlacementRootKind.Aisles,
                includeScale: true);
            y += captureHeight;

            y = DrawChild(property, y, position.x, position.width, "category");
            y = DrawChild(property, y, position.x, position.width, "label");
            ...
            y = DrawChild(property, y, position.x, position.width, "localScale");
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        ...
        float line = EditorGUIUtility.singleLineHeight;
        float gap = EditorGUIUtility.standardVerticalSpacing;
        float h = line + gap; // foldout
        h += line + gap + line + gap; // capture block
        h += GetChildHeight(property, "category");
        h += GetChildHeight(property, "label");
        h += GetChildHeight(property, "prefab");
        h += GetChildHeight(property, "localPosition");
        h += GetChildHeight(property, "localEulerAngles");
        h += GetChildHeight(property, "localScale");
        return h;
    }
